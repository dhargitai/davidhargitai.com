export function NextButton({ onClick, style, className }: { onClick?: () => void, style?: { [key: string]: string }, className?: string}) {
  return (
    <div
      onClick={onClick}
      className={`absolute z-10 mr-0 h-12 w-12 md:w-20 md:h-20 max-w-20 cursor-pointer select-none overflow-hidden bg-gray-100 text-2xl leading-4 text-slate-900 hover:bg-slate-900 hover:text-white left-0 right-0 top-0 bottom-0 ${className}`}
      role="button"
      tabIndex={0}
